...
async fn main() -> anyhow::Result<()> {
    ...
    stream::iter(root_space_ops)
        ...
        .try_for_each(|op| async move { op.apply_op(kg_ref, ROOT_SPACE_ID).await })
        .await?;

    // // Import construction space ops
    let construction_space_import = import_space(&ipfs_client, CONSTRUCTION_SPACE_IMPORTS).await?;
    ...
    stream::iter(construction_space_ops)
        ...
        .try_for_each(|op| async move {
            op.apply_op(kg_ref, "0x74519E6EEc5BCFBC4Eb8F1A6d0C6D343173A286b")
                .await
        })
        .await?;
