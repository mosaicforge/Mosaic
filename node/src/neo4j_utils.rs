...
pub trait Neo4jExt {
    ...
    fn insert_one<T: Serialize + Send>(&self, node: T) -> impl std::future::Future<Output = anyhow::Result<()>> + Send;

    fn insert_many<T: Serialize>(&self, node: Vec<T>) -> impl std::future::Future<Output = anyhow::Result<()>>;
}
...
impl Neo4jExt for neo4rs::Graph {
    ...
    async fn insert_one<T: Serialize>(&self, node: T) -> anyhow::Result<()> {
        ...
        let query = query
            .param("node", serde_value_to_bolt(serde_json::to_value(&node)?));
    ...
    async fn insert_many<T: Serialize>(&self, node: Vec<T>) -> anyhow::Result<()> {
        ...
        let query = if let Some(label) = label {
            neo4rs::query(&format!("UNWIND $nodes AS node CREATE (n:{label}) SET n = node"))
        } else {
            neo4rs::query("UNWIND $nodes AS node CREATE (n) SET n = node")
        };

        let query = query
            .param("nodes", serde_value_to_bolt(json));
...
pub trait Neo4jMutExt {
    ...
    fn insert_one<T: Serialize + Send>(&mut self, node: T) -> impl std::future::Future<Output = anyhow::Result<()>> + Send;

    fn insert_many<T: Serialize>(&mut self, node: Vec<T>) -> impl std::future::Future<Output = anyhow::Result<()>>;
}
...
fn serde_value_to_bolt(value: serde_json::Value) -> BoltType {
    match value {
        ...
        serde_json::Value::Number(number) if number.is_i64() => {
            BoltType::Integer(neo4rs::BoltInteger { value: number.as_i64().expect("number should be an i64") })
        },
        serde_json::Value::Number(number) if number.is_u64() => {
            BoltType::Integer(neo4rs::BoltInteger { value: number.as_u64().expect("number should be a u64") as i64 })
        },
        serde_json::Value::Number(number) => {
            BoltType::Float(neo4rs::BoltFloat { value: number.as_f64().expect("number should be an f64") })
        },
        ...
        serde_json::Value::Object(map) => {
            let properties = map.into_iter()
                .filter(|(key, _)| key != "$type")
                .map(|(key, value)| (neo4rs::BoltString { value: key }, serde_value_to_bolt(value)))
                .collect();
...
mod tests {
    ...
    async fn test_find_all_relations() {
        ...
        let query = neo4rs::query("MATCH () -[r:`01412f8381894ab1836565c7fd358cc1`]-> () RETURN r LIMIT 4");
    ...
    async fn test_find_one_node_txn() {
        ...
        txn.run(neo4rs::query("CREATE (n:TestNode {name: \"potato\"})")).await.unwrap();
    ...
    async fn test_insert_one_txn() {
        ...
        let node = Foo { name: "potato".to_string() };

        txn.insert_one(node).await.unwrap();

        let node: Option<Foo> = txn.find_one(neo4rs::query("MATCH (n:Foo) RETURN n LIMIT 1")).await.unwrap();
    ...
    async fn test_insert_many_txn() {
        ...
        let nodes = vec![
            Foo { name: "potato".to_string() },
            Foo { name: "pizza".to_string() },
        ];

        txn.insert_many(nodes).await.unwrap();

        let node: Vec<Foo> = txn.find_all(neo4rs::query("MATCH (n:Foo) RETURN n")).await.unwrap();
