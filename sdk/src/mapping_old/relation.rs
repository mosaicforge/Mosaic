...
use super::{attributes::SystemProperties, relation_queries, triple::{TryFromTriples, TryIntoTriples}, Entity, Triples};
...
impl<T> Relation<T> {
    ...
    pub fn from_entity(entity: Entity<T>, from: &str, to: &str, relation_type: &str) -> Self {
        Self {
            from: from.to_string(),
            to: to.to_string(),
            relation_type: relation_type.to_string(),
    ...
    T: Clone + TryIntoTriples,
{
    ...
    pub async fn upsert(&self, neo4j: &neo4rs::Graph) -> Result<(), DatabaseError> {
        const QUERY: &str = const_format::formatcp!(
            ...
            MERGE (relation_type {{id: $relation_type_id}})
            ...
            MERGE (from)<-[:`{FROM_ENTITY}` {{space_id: $space_id, version_index: $version_index}}]-(r)
            MERGE (r)-[:`{TO_ENTITY}` {{space_id: $space_id, version_index: $version_index}}]->(to)
            MERGE (r) -[:`{RELATION_TYPE}` {{space_id: $space_id, version_index: $version_index}}]-> (relation_type)
            WITH r
            UNWIND $triples AS triple
            MERGE (r) -[r:ATTRIBUTE {{attribute: triple.attribute, space_id: $space_id, version_index: $version_index}}]-> (m)
            SET m += triple
            "#,
            FROM_ENTITY = system_ids::RELATION_FROM_ATTRIBUTE,
            TO_ENTITY = system_ids::RELATION_TO_ATTRIBUTE,
        ...
        let triples = self.entity.attributes.clone().try_into_triples()?;
        ...
        let query = neo4rs::query(QUERY)
            ...
            .param("relation_type_id", self.relation_type.clone())
            .param("r_types", self.entity.types.clone())
            .param(
                "from_types",
                if self.relation_type == system_ids::TYPES_ATTRIBUTE {
                    vec![self.to.clone()]
                } else {
                    vec![]
                },
