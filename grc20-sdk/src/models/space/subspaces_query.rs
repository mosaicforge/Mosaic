...
use grc20_core::{
    ...
    error::DatabaseError,
    indexer_ids,
    mapping::{aggregation::SpaceRanking, prop_filter, query_utils::{query_builder::{QueryBuilder, Subquery}, QueryStream}, PropFilter, RelationEdge},
    neo4rs, relation,
};

/// Query to find all subspaces of a given space
pub struct SubspacesQuery<T> {
    neo4j: neo4rs::Graph,
    space_id: String,
    limit: usize,
    skip: Option<usize>,
...
impl QueryStream<SpaceRanking> for SubspacesQuery<SpaceRanking> {
    async fn send(
        self,
