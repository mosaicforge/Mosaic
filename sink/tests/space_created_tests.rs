use grc20_core::{
    entity::Entity, indexer_ids, mapping::Query, network_ids, pb::geo,
};
...
async fn test_handle_space_created() {
    ...
    // Call handler
    let space_id = handler.handle_space_created(&space_created, &[], &block).await.unwrap();
    ...
    let expected = Entity::new(space_id, space::Space {
...
async fn test_handle_personal_space_created() {
    ...
    handler.handle_personal_space_created(&personal_space_created, &block).await.unwrap();
    ...
    let expected = Entity::new(space_id, space::Space {
        ...
        personal_space_admin_plugin: Some(checksum_address(&personal_space_created.personal_admin_address)),
        voting_plugin_address: None,
        member_access_plugin: None,
    });
...
async fn test_handle_governance_plugin_created() {
    ...
    handler.handle_governance_plugin_created(&governance_plugin_created, &block).await.unwrap();
    ...
    let expected = Entity::new(space_id, space::Space {
        ...
        governance_type: SpaceGovernanceType::Public,
        personal_space_admin_plugin: None,
        voting_plugin_address: Some(checksum_address(&governance_plugin_created.main_voting_address)),
        member_access_plugin: Some(checksum_address(&governance_plugin_created.member_access_address)),
    });
